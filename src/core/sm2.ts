import { CardState, Rating } from '../types/index.js';
...
export interface SM2Output {
  ...
  interval: number;
  dueDate: string;
  /** Whether this card has graduated (left learning/relearning) this review */
  graduated: boolean;
}
 ...
 */
export function getIntervalHint(input: SM2Input): Record<Rating, string> {
  const hints: Record<Rating, string> = { 1: '', 2: '', 3: '', 4: '' };
  ...
  for (const r of [1, 2, 3, 4] as Rating[]) {
    ...
    if (result.state === 'learning' || result.state === 'relearning') {
      ...
      // Interval is in days
      if (result.interval === 1) hints[r] = '1d';
      else hints[r] = `${result.interval}d`;
    }
  }
...
function handleNew(rating: Rating, easeFactor: number): SM2Output {
  switch (rating) {
...
function handleLearning(rating: Rating, step: number, easeFactor: number): SM2Output {
...
function handleReview(
  ...
  if (rating === 4) newInterval = Math.max(1, Math.round(newInterval * 1.3));
...
function graduate(reps: number, easeFactor: number): SM2Output {
  return {
    state: 'review',
    learningStep: 0,
    repetitions: reps + 1,
