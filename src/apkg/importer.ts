...
export async function importApkg(apkgPath: string): Promise<{ decks: number; cards: number }> {
  ...
  try {
    ...
    const existingDecksByName = new Map<string, number>(
      getAllDecks().map(d => [d.name, d.id])
    );
      ...
      for (const d of ankiDecks) {
        ...
        try {
          ...
          if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') {
            ...
            if (existingId !== undefined) deckIdMap.set(String(d.id), existingId);
          }
        }
      }
    } else {
      ...
      for (const [ankiDeckId, ankiDeck] of Object.entries(ankiDecks)) {
        ...
        try {
          ...
          if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') {
            ...
            if (existingId !== undefined) deckIdMap.set(ankiDeckId, existingId);
          }
        }
      }
    }
