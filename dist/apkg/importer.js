...
export async function importApkg(apkgPath) {
    ...
    try {
        ...
        for (const card of cards) {
            ...
            if (frontImage)
                copyMedia(frontImage, mediaMap, tmpDir);
            if (backImage)
                copyMedia(backImage, mediaMap, tmpDir);
            // For image-only cards, use a placeholder so the card isn't silently dropped
            const front = stripHtml(frontRaw) || (frontImage ? '(image)' : '');
            const back = stripHtml(backRaw) || (backImage ? '(image)' : '');
            if (!front || !back)
                continue;
            const deckId = deckIdMap.get(String(card.did));
            if (!deckId)
                continue;
            createCard(deckId, front, back, frontImage, backImage);
            cardCount++;
        }
        ankiDb.close();
...
function stripHtml(html) {
    ...
    return html
        .replace(/<br\s*\/?>/gi, '\n')
        .replace(/<img[^>]+>/gi, '') // Special handling for images: remove them but keep them in createCard
        .replace(/<[^>]+>/g, '')
        .replace(/&nbsp;/g, ' ')
        .replace(/&amp;/g, '&')
        .replace(/&lt;/g, '<')
...
function copyMedia(filename, mediaMap, tmpDir) {
 ...
 */
function readMediaManifest(mediaPath, tmpDir) {
 ...
 */
function matchMediaBySha1(entries, tmpDir) {
    const result = {};
    // Index proto entries by their sha1 hex for O(1) lookup
    const bySha1 = new Map();
    for (const e of entries) {
        if (e.sha1)
            bySha1.set(e.sha1.toString('hex'), e.name);
    }
 ...
 */
function parseMediaProtobuf(buf) {
    const result = [];
    let pos = 0;
    function readVarint() {
        let value = 0, shift = 0;
